...
# Build
FROM docker.io/golang:1.23.3-bookworm AS build
...
RUN curl --fail --silent --show-error --location "https://raw.githubusercontent.com/golangci/golangci-lint/master/install.sh"|sh -s -- -b "$(go env GOPATH)"/bin v1.62.0 \
  && golangci-lint run -v
